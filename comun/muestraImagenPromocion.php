<?php
	...
	$SELECT ="";	
	if(ISSET($_GET["cod"])){
		$SELECT ="SELECT ID,VALIDEZ,VIGENCIA,PRINCIPAL,FOTO FROM brc_promociones_web WHERE ID=".$_GET["cod"]."";
	}else{
		$SELECT ="SELECT TOP(1) ID,VALIDEZ,VIGENCIA,PRINCIPAL,FOTO FROM brc_promociones_web ";
	}
	$config = new Vars($dt);
	$db = new db();
